...
const computeInitialPriority = async (productId) => {
  ...
  if (productImageCache.has(productId)) {
    ...
    return (
      openModalVelocity + (likeVelocity + saveVelocity + dislikeVelocity) * 0.5
...
const getProductImage = async (userId, productId) => {
  ...
  try {
    ...
    if (productImageCache.has(productId)) {
      if (
        Date.now() - productImageCache.get(productId).timestamp.getTime() >=
        TTL
      ) {
