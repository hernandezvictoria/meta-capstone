...
const parseLikedDislikedProducts = (products) => {
    ...
    for (const brand of brands) {
        ...
        if (brandFrequencies[brand] >= MIN_BRANDS) {
            // if brand appears MIN_BRANDS or more times, add to set
            brandSet.add(brand);
        }
    }
    ...
    for (const ingredient of ingredients) {
        ingredientFrequencies[ingredient] =
            (ingredientFrequencies[ingredient] || 0) + 1;
        if (ingredientFrequencies[ingredient] >= MIN_INGREDIENTS) {
            // if ingredient appears MIN_INGREDIENTS or more times, add to set
    ...
    for (const skinType of userSkinType) {
        ...
        if (
            product.ingredients.some((ingredient) =>
                ingredient.skin_type.includes(skinType)
            )
        ) {
...
const computeLikedDislikedOverlapScore = (product, likedDislikedProducts) => {
    const parsedBrandsAndIngredients = parseLikedDislikedProducts(
        likedDislikedProducts
    );
    ...
    for (const lovedProduct of likedDislikedProducts) {
        ...
        if (lovedProduct.brand === product.brand) {
            likedDislikedProductOverlapScore += likedDislikedBrands.has(
                lovedProduct.brand
            )
                ? LIKED_DISLIKED_BRAND_BOOST * 2
                : LIKED_DISLIKED_BRAND_BOOST; // double boost if brand is well-liked/well-disliked
        }
    ...
    if (!isProductLikedDisliked && likedDislikedProducts.length > 0) {
        likedDislikedProductOverlapScore +=
            likedDislikedProductIngredientSimilarityScore /
            likedDislikedProducts.length; // average jaccard score
    }
    ...
    likedDislikedProductOverlapScore = Math.min(
        likedDislikedProductOverlapScore,
        MAX_LIKED_DISLIKED_OVERLAP_SCORE
    ); // cap at MAX_LIKED_DISLIKED_OVERLAP_SCORE
...
const computeProductScore = (
    ...
    const { productSkinTypeScore, ingredientSkinTypeScore } =
        computeSkinTypeScore(product, userSkinType);
    const { productConcernsScore, ingredientConcernsScore } =
        computeConcernScore(product, userSkinConcerns);
    ...
    let lovedProductOverlapScore = computeLikedDislikedOverlapScore(
        product,
        lovedProducts
    );
    let dislikedProductOverlapScore = computeLikedDislikedOverlapScore(
        product,
        dislikedProducts
    );
