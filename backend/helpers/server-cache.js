...
const resizeCache = () => {
    ...
    dynamicCacheLifetime =
        ONE_DAY_MILLISECONDS / (dynamicCacheSize / MIN_CACHE_SIZE); // larger caches have shorter lifetimes
...
const setUserActivity = async () => {
    ...
    const userLoginsInPastWeek = allUserLogins.filter(
        (login) =>
    ...
    const userLogoutsInPastHour = allUserLogouts.filter(
        (logout) =>
    ...
    for (const loginActivity of userLoginsInPastWeek) {
        if (
            Date.now() - loginActivity.activity_time.getTime() <=
            ONE_HOUR_MILLISECONDS
        ) {
            // if activity happened in last hour
            // if the user hasn't logged out yet
            if (
                !userLogoutsInPastHour.some((logoutActivity) => {
                    logoutActivity.user_id === loginActivity.user_id &&
                        logoutActivity.activity_time.getTime() >
                            loginActivity.activity_time.getTime(); // user logged out after logging in
                })
            ) {
                numActiveUsers++;
            }
        ...
        if (userToActivityScore.has(loginActivity.user_id)) {
            userToActivityScore.set(
                loginActivity.user_id,
                userToActivityScore.get(loginActivity.user_id) + 1
            );
...
const getInteractionScores = (userClicksInLastDay) => {
    ...
    const incrementFrequency = (interactionType, userId) => {
        ...
        if (interactionUsers.has(interactionType)) {
            ...
            // first time interaction is being added to the maps
            userSet = new Set();
            userSet.add(userId);
        }
        ...
        if (!isUserInSet) {
            // if user not already documented for this interaction, add user score to interactionScores
            const userActivityScore = userToActivityScore.get(userId);
            if (interactionScores.has(interactionType)) {
                interactionScores.set(
                    interactionType,
                    interactionScores.get(interactionType) + userActivityScore
                ); // add user's score to existing score sum
            } else {
...
const computeInitialPriority = async (productId) => {
    ...
    const likeScore =
        interactionScores.get(InteractionTypes.LIKE) -
        interactionScores.get(InteractionTypes.REMOVE_LIKE);
    const saveScore =
        interactionScores.get(InteractionTypes.SAVE) -
        interactionScores.get(InteractionTypes.REMOVE_SAVE);
    const dislikeScore =
        interactionScores.get(InteractionTypes.DISLIKE) -
        interactionScores.get(InteractionTypes.REMOVE_DISLIKE);
...
const getProductImage = async (userId, productId) => {
    if (!currentUserId || !productImageCache || !productQueue) {
        // create cache if they don't exist
        createQueueAndCache();
        potentialAPICalls = 0;
        cacheHits = 0;
        resetCounters();
    ...
    try {
        ...
        if (productImageCache.has(productId)) {
            if (
                Date.now() -
                    productImageCache.get(productId).timestamp.getTime() >=
                dynamicCacheLifetime
            ) {
                ...
                const removed = productQueue.remove(
